...
def segmenter_par_parole(texte, taille_max=5000):  # réduit pour éviter les erreurs 413
...
# ----- INTERACTIF -----
nom_entretien = input("Nom du fichier PDF (ex: D2SN11 ou Lau4) : ").strip()
chemin_pdf = os.path.join(dossier_pdf, f"{nom_entretien}.pdf")

texte = lire_pdf(chemin_pdf)
if not texte:
    print(f"⚠ PDF vide ou introuvable : {nom_entretien}.pdf")
    exit()
