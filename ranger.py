...
def charger_et_parser_fichiers(entretien, dossier="resultats"):
    pattern = os.path.join(dossier, f"{entretien}_segment*.txt")  # correction ici
...
def nettoyer_et_exporter(df, entretien):
    ...
    print(f"Nombre de lignes (codes uniques) : {len(df_clean)}")
    print("Thèmes détectés :", df_clean['Thème'].nunique())

if __name__ == "__main__":  # correction ici
    nom_entretien = input("Nom de l'entretien (ex: D2SN4) : ").strip()
